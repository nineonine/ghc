...
matchExpectedFunTys herald arity orig_ty thing_inside
  ...
  where
    ...
    go acc_arg_tys n (FunTy arg_ty res_ty) ctx
      = ASSERT( not (isPredTy arg_ty) )
        do { (result, wrap_res) <- go ((mkCheckExpType arg_ty $ text "matchExpectedFunTys2") : acc_arg_tys)
                                      (n-1) res_ty ctx
           ; return ( result
    ...
    go acc_arg_tys n ty@(TyVarTy tv) ctx
      ...
      = do { cts <- readMetaTyVar tv
           ; case cts of
               ...
               Flexi        -> defer acc_arg_tys n (mkCheckExpType ty $ text "matchExpectedFunTys isMetaTyVar Flexi") }
    ...
    go acc_arg_tys n ty _ = addErrCtxtM mk_ctxt $
                            defer acc_arg_tys n (mkCheckExpType ty $ text "matchExpectedFunTys defer case")
...
tcSkolemiseET ctxt (Check ty cctx) thing_inside
  = tcSkolemise ctxt ty $ \_  tcTy -> thing_inside $ mkCheckExpType tcTy $ cctx -- text "tcSkolemiseET"
